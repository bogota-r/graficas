...
ls(pattern = '^stat_', env = as.environment('package:ggplot2')) %>%
  setdiff("stat_abline") %>% matrix(6) %>% kable
```

## `r geom_slide_title("geom_violin")` (stat_ydensty)

```{r geom_violin}
ggplot(mtcars) + aes(y = mpg, x = factor(cyl)) + geom_violin()
```

## `r geom_slide_title("geom_dotplot")` (stat_bindot)
...
## `r geom_slide_title("geom_point")` (stat_qq)
...
## `r geom_slide_title("geom_point")`

```{r geom_point}
ggplot(diamonds) + aes(price, depth) + geom_point()
```

## `r geom_slide_title("geom_bin2d")` (stat_bin2d)

```{r geom_bin2d}
ggplot(diamonds) + aes(price, depth) + geom_bin2d()
```

## `r geom_slide_title("geom_bin2d")` (stat_summary2d)
...
## `r geom_slide_title("geom_hex")` (stat_binhex)
...
## `r geom_slide_title("geom_hex")` (stat_summary_hex)
...
## `r geom_slide_title("geom_points")`

```{r geom_points2}
ggplot(mpg) + aes(displ, hwy) + geom_point()
```

## `r geom_slide_title("geom_points")` (stat_spoke)

```{r geom_points2_spoke}
ggplot(mpg) + aes(displ, hwy, angle = runif(234, 0, 2*pi)) + geom_point() + stat_spoke(radius = 0.5)
```

## `r geom_slide_title("geom_jitter")` (the overploting problem)
...
## `r geom_slide_title("geom_quantile")` (stat_quantile)

```{r geom_quantile}
ggplot(mpg) + aes(displ, hwy) + geom_point() + geom_quantile()
```

## `r geom_slide_title("geom_smooth")` (stat_smooth)

```{r geom_smooth}
ggplot(mpg) + aes(displ, hwy) + geom_point() + geom_smooth()
```

## `r geom_slide_title("geom_density2d")` (stat_density2d)
...
## `r geom_slide_title("geom_crossbar")` 
...
## `r geom_slide_title("geom_linerange")`
...
## `r geom_slide_title("geom_pointrange")`
...
## `r geom_slide_title("geom_errorbar")`
...
## `r geom_slide_title("geom_errorbarh")`
...
## `r geom_slide_title("geom_ribbon")`
...
## `r geom_slide_title("geom_density")` (stat_density)

```{r geom_density}
ggplot(mtcars) + aes(x = mpg) + geom_density()
```

## `r geom_slide_title("geom_density")` (stat_density + stat_function)
...
## `r geom_slide_title("geom_freqpoly")` (stat_bin)

```{r geom_freqpoly}
ggplot(mtcars) + aes(x = mpg) + geom_freqpoly(binwidth = 2)
```

## `r geom_slide_title("geom_histogram")` (stat_bin)

```{r geom_histogram}
ggplot(mtcars) + aes(x = mpg) + geom_histogram(binwidth = 2)
```

## `r geom_slide_title("geom_abline")` (stat_abline)
...
```{r geom_blank-a, img.link = "http://docs.ggplot2.org/0.9.3.1/geom_blank.html"}
ggplot(mtcars) + aes(x = wt, y = mpg, colour = factor(cyl), size = hp) + geom_blank() +
  geom_abline(aes(intercept = a, slope = b), data = df1)
```

## `r geom_slide_title("geom_segment")`
...
## `r geom_slide_title("geom_polygon")`
...
## `r geom_slide_title("geom_raster")` 
...
## `r geom_slide_title("geom_countour")`
